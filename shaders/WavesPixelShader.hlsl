...
{
    shaderTexture.Sample(samplerState, input.TexCoord);
    float waveShift = sin(input.TexCoord.y * 20) * 0.1;
    float4 texColor = shaderTexture.Sample(samplerState, float2(input.TexCoord.x + waveShift, input.TexCoord.y));
